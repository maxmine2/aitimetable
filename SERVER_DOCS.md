...
Каждый входящий запрос проходит через метод `_guard()`, который последовательно применяет три проверки. При срабатывании любой из них запрос отклоняется, а остальная логика не исполняется.

### 7.1. Блокировка User-Agent
...
**Реакция**: Ответ: `403 Forbidden`.

### 7.2. Блокировка подозрительных URL-паттернов
...
### 7.3. Ограничение размера тела запроса
...
При запуске с `--host 0.0.0.0` сервер слушает на всех интерфейсах и доступен из локальной сети. В этом случае защита обеспечивается остальными уровнями (блокировка UA/URL, белый список файлов).
