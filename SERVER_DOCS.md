...
```bash
python serve.py            # порт по умолчанию (8764)
python serve.py -p 9000    # произвольный порт
python serve.py --port 9000
```

Сервер стартует на `http://localhost:<port>` и автоматически открывает `timetable.html` в браузере.
...
| Привязка | `127.0.0.1` | Только localhost (недоступен извне) |
...
### 8.1. Привязка к localhost

```python
http.server.HTTPServer(("127.0.0.1", PORT), Handler)
```

Сервер принимает подключения **только** с `127.0.0.1`. Запросы с внешних IP-адресов физически невозможны на сетевом уровне — они отклоняются ОС ещё до попадания в Python.
